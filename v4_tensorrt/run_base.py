"""
v4_tensorrt/run_base - Basic TRT (synchronous, no threading)
- Webcam: cv2.VideoCapture direct (no threading)
- Inference: synchronous in main loop
- Same structure as v1_baseline but with TRT engine loading
...
def parse_args():
    parser = argparse.ArgumentParser(description="v4_tensorrt/run_base: Basic TRT (synchronous, no threading)")
...
def main():
    ...
    print("=" * 60)
    print("v4_tensorrt/run_base - Basic TRT (sync, no threading)")
    print("=" * 60)
    ...
    while True:
        ...
        if test_mode:
            ...
            cv2.imshow("Compare (v4_tensorrt/run_base)", combine_frame)
    ...
    if test_mode:
        print("=" * 40)
        print(f"  v4_tensorrt/run_base Benchmark Result")
