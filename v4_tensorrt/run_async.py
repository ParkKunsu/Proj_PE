"""
v4_tensorrt/run_async - Async inference + TRT
- Webcam: cv2.VideoCapture direct (no threading)
- Inference: AsyncPoseInference in separate thread
- FPS: Display FPS + Inference FPS separated
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="v4_tensorrt/run_async: Async inference + TRT")
...
def main():
    ...
    print("=" * 60)
    print("v4_tensorrt/run_async - Async Inference + TRT")
    print("=" * 60)
    ...
    # Load input source (direct cv2.VideoCapture, no threading)
    use_image = False
    use_webcam = False
    ...
    if args.image:
        ...
        if use_webcam:
            print("Input: webcam (direct, no threading)")
        else:
    ...
    while True:
        ...
        if test_mode:
            ...
            cv2.imshow("Compare (v4_tensorrt/run_async)", combine_frame)
    ...
    if test_mode:
        print("=" * 40)
        print(f"  v4_tensorrt/run_async Benchmark Result")
